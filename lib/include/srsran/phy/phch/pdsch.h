...
typedef struct {
  srsran_sequence_t seq[SRSRAN_MAX_CODEWORDS][SRSRAN_NOF_SF_X_FRAME];
  uint32_t          cell_id;
...
SRSRAN_API int srsran_pdsch_set_rnti(srsran_pdsch_t* q, uint16_t rnti);

SRSRAN_API void srsran_pdsch_free_rnti(srsran_pdsch_t* q, uint16_t rnti);
