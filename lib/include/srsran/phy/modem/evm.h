 ...
 */
static inline srsran_evm_buffer_t* srsran_evm_buffer_alloc(uint32_t nof_prb)
{
  srsran_evm_buffer_t* q = (srsran_evm_buffer_t*)srsran_vec_malloc(sizeof(srsran_evm_buffer_t));
  ...
  memset(q, 0, sizeof(srsran_evm_buffer_t));

  // Set max number of bits
  q->max_bits = srsran_ra_tbs_from_idx(SRSRAN_RA_NOF_TBS_IDX - 1, nof_prb);
 ...
 * @return EVM buffer pointer
 */
static inline void srsran_evm_buffer_resize(srsran_evm_buffer_t* q, uint32_t nof_prb)
{
  ...
  uint32_t new_max_bits = srsran_ra_tbs_from_idx(SRSRAN_RA_NOF_TBS_IDX - 1, nof_prb);
