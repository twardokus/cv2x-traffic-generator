...
public:
   ...
   */
  explicit rf_buffer_t(cf_t* data[SRSRAN_MAX_CHANNELS])
  {
    for (uint32_t i = 0; i < SRSRAN_MAX_CHANNELS; i++) {
...
{
public:
  radio(srsran::log_filter* log_h);
  radio(srsran::logger* logger_h);
  virtual ~radio();
  ...
  bool tx(rf_buffer_interface& buffer, const uint32_t& nof_samples, const srsran_timestamp_t& tx_time) override;
  bool rx_now(rf_buffer_interface& buffer, const uint32_t& nof_samples, srsran_timestamp_t* rxd_time) override;
  ...
  float get_rssi();
  bool  has_rssi();
  void  get_time(srsran_timestamp_t* now) override;
...
private:
  ...
  srsran_timestamp_t end_of_burst_time  = {};
