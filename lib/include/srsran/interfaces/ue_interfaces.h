...
{
public:
  virtual void start_attach_proc(srsran::proc_state_t* proc_result, srsran::establishment_cause_t cause_) = 0;
...
public:
  ...
  virtual void write_sdu(uint32_t lcid, srsran::unique_byte_buffer_t sdu, bool blocking)                            = 0;
...
public:
  ...
  virtual void write_sdu(uint32_t lcid, srsran::unique_byte_buffer_t sdu, bool blocking = true) = 0;
};
...
public:
  ...
  typedef struct {
    mac_tb_t     tb[SRSRAN_MAX_NUM_SUB_CHANNEL];
    srsran_sci_t sci[SRSRAN_MAX_NUM_SUB_CHANNEL];
    uint32_t     rssi[SRSRAN_MAX_NUM_SUB_CHANNEL];
    uint32_t     tti;
  } mac_grant_sl_rx_t;
 ...
 * payload_ptr points to memory where MAC PDU must be written by MAC layer */
  ...
  /* Indicate reception of SL. */
  virtual void new_grant_sl_rx(mac_grant_sl_rx_t grant, tb_action_sl_t action[SRSRAN_MAX_NUM_SUB_CHANNEL]) = 0;
...
typedef struct {
  ...
  srsran::channel::args_t sl_rx_channel_args;
  srsran::channel::args_t sl_tx_channel_args;
} phy_args_t;
...
{
public:
  virtual void set_config(srsran::phy_cfg_t& config,
                          uint32_t           cc_idx    = 0,
                          uint32_t           earfcn    = 0,
                          srsran_cell_t*     cell_info = nullptr) = 0;
...
class stack_interface_rrc : public srsran::task_handler_interface
{
public:
