...
struct state_traits {
  ...
  using state_t   = State;
  using is_subfsm = std::integral_constant<bool, ::srsran::fsm_details::is_subfsm<State>()>;
...
public:
  ...
  explicit fsm_t(srsran::log_ref log_) : log_h(log_) {}
  ...
  void set_fsm_event_log_level(srsran::LOG_LEVEL_ENUM e) { fsm_event_log_level = e; }

  srsran::log_ref get_log() const { return log_h; }
...
protected:
  ...
  srsran::LOG_LEVEL_ENUM fsm_event_log_level = LOG_LEVEL_INFO;
};
...
public:
  ...
  explicit proc_fsm_t(srsran::log_ref log_) : fsm_t<Derived>(log_) {}
