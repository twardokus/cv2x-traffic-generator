...
public:
  ...
  void resize(size_t new_size) noexcept
  {
    if (new_size > max_size()) {
      srsran::logmap::get("COMM")->error("ERROR: bitset resize out of bounds: %zd>=%zd\n", max_size(), new_size);
      return;
  ...
  {
    if (pos >= size()) {
      srsran::logmap::get("COMM")->error("ERROR: bitset out of bounds: %zd>=%zd\n", pos, size());
      return;
    }
    if (val) {
      set_(pos);
  ...
  {
    if (nof_words_() > 1) {
      srsran::logmap::get("COMM")->error("ERROR: cannot convert bitset of size %zd bits to uint64_t\n", size());
      return 0;
    }
    return get_word_(0);
  }
