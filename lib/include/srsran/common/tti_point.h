  ...
  explicit tti_point(uint32_t tti_val_) : tti_val(tti_val_ % 10240)
  {
    if (tti_val_ > std::numeric_limits<uint32_t>::max() / 2) {
      ...
      if (diff < 10240) {
        ...
        srsran::logmap::get("COMMON")->error("Invalid TTI point assigned\n");
      }
    }
  }
...
inline tti_point to_tx_dl(const srsran::tti_point& t)
{
  return t + FDD_HARQ_DELAY_UL_MS;
}
inline tti_point to_tx_ul(const srsran::tti_point& t)
{
  return t + (FDD_HARQ_DELAY_UL_MS + FDD_HARQ_DELAY_DL_MS);
}
inline tti_point to_tx_dl_ack(const srsran::tti_point& t)
{
  return to_tx_ul(t);
}
