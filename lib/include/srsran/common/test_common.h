...
class test_log_filter : public srsran::log_filter
{
public:
  explicit test_log_filter(std::string layer) : srsran::log_filter(std::move(layer))
  {
    set_level(srsran::LOG_LEVEL_DEBUG);
  }
  ...
  {
    warn_counter++;
    if (level >= srsran::LOG_LEVEL_WARNING) {
      char    args_msg[char_buff_size];
      va_list args;
      va_start(args, message);
      if (vsnprintf(args_msg, char_buff_size, message, args) > 0) {
        all_log(srsran::LOG_LEVEL_WARNING, tti, args_msg);
      }
      va_end(args);
    }
  }
...
public:
  ...
  srsran::LOG_LEVEL_ENUM last_log_level = LOG_LEVEL_NONE;
  std::string            last_log_msg;

private:
  void log_va_list(srsran::LOG_LEVEL_ENUM loglevel, const char* message, va_list argp)
  {
...
public:
  ...
  explicit scoped_log(Args&&... args)
  {
    ...
    prev_log    = srsran::logmap::deregister_log(l->get_service_name());
    current_log = l.get();
    srsran::logmap::register_log(std::move(l));
  }
  scoped_log(scoped_log<Log>&&) noexcept = default;
  ~scoped_log()
  {
    srsran::logmap::deregister_log(current_log->get_service_name());
    if (prev_log != nullptr) {
      srsran::logmap::register_log(std::move(prev_log));
    }
  }
...
private:
  ...
  std::unique_ptr<srsran::log> prev_log;
};
...
#define TESTERROR(fmt, ...)                                                                                            \
  do {                                                                                                                 \
    srsran::logmap::get("TEST")->error(fmt, ##__VA_ARGS__);                                                            \
    return SRSRAN_ERROR;                                                                                               \
  } while (0)

#define TESTWARN(fmt, ...)                                                                                             \
  do {                                                                                                                 \
    srsran::logmap::get("TEST")->warning(fmt, ##__VA_ARGS__);                                                          \
  } while (0)
