...
private:
  ...
  srsran::log_filter* log_h = nullptr;
  std::mutex          mutex;
...
public:
  dummy_radio(srsran::log_filter& log_h_) : log_h(&log_h_) { log_h->info("Dummy radio created\n"); }
...
int main(int argc, char** argv)
{
  ...
  // Loggers
  srsran::logger_stdout                             logger;
  srsran::log_filter                                radio_log("radio", &logger);
  std::vector<std::unique_ptr<srsran::log_filter> > worker_logs;
  ...
  for (uint32_t i = 0; i < nof_workers; i++) {
    ...
    // Create log filter
    srsran::log_filter* log_filter = new srsran::log_filter(log_name, &logger);
    ...
    // Push back objects
    worker_logs.push_back(std::unique_ptr<srsran::log_filter>(log_filter));
