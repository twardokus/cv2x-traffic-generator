                                           ...
                                           float noise_estimate)
{
  ...
  for (; i < nof_symbols; i++) {
    ...
    h01 = +h[0][0][i] - h[1][0][i];
    h11 = +h[0][1][i] - h[1][1][i];
    srsran_mat_2x2_mmse_gen(y[0][i], y[1][i], h00, h01, h10, h11, &x[0][i], &x[1][i], noise_estimate, 2.0f / scaling);
...
float srsran_precoding_2x2_cn_gen(cf_t* h[SRSRAN_MAX_PORTS][SRSRAN_MAX_PORTS], uint32_t nof_symbols)
{
  ...
  for (uint32_t i = 0; i < nof_symbols; i += PMI_SEL_PRECISION) {
    ...
    cn_avg += srsran_mat_2x2_cn(h00, h01, h10, h11);
