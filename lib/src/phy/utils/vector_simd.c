...
void srsran_vec_xor_bbb_simd(const int8_t* x, const int8_t* y, int8_t* z, const int len)
{
  int i = 0;
...
#if SRSRAN_SIMD_CF_SIZE
  simd_f_t _sum_sin = srsran_simd_f_zero();

  for (; i < len - SRSRAN_SIMD_CF_SIZE + 1; i += SRSRAN_SIMD_CF_SIZE) {
    simd_cf_t a1  = srsran_simd_cfi_loadu(&x[i]);
    simd_f_t  re1 = srsran_simd_cf_re(a1);
    simd_f_t  im1 = srsran_simd_cf_im(a1);

    simd_cf_t a2  = srsran_simd_cfi_loadu(&x[i - 1]);
    simd_f_t  re2 = srsran_simd_cf_re(a2);
    simd_f_t  im2 = srsran_simd_cf_im(a2);

    simd_f_t _pow = srsran_simd_f_sqrt(
        srsran_simd_f_mul(srsran_simd_f_add(srsran_simd_f_mul(re1, re1), srsran_simd_f_mul(im1, im1)),
                          srsran_simd_f_add(srsran_simd_f_mul(re2, re2), srsran_simd_f_mul(im2, im2))));

    simd_f_t _sin = srsran_simd_f_mul(srsran_simd_f_sub(srsran_simd_f_mul(re1, im2), srsran_simd_f_mul(re2, im1)),
                                      srsran_simd_f_rcp(_pow));
    _sum_sin      = srsran_simd_f_add(_sum_sin, _sin);
  }

  float _sum_sin_v[SRSRAN_SIMD_CF_SIZE];
  srsran_simd_f_storeu(_sum_sin_v, _sum_sin);
  for (int k = 0; k < SRSRAN_SIMD_CF_SIZE; k++) {
