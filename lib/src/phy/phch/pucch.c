...
int srsran_pucch_init_(srsran_pucch_t* q, bool is_ue)
{
  ...
  if (q != NULL) {
    ...
    q->users = calloc(sizeof(srsran_pucch_user_t*), q->is_ue ? 1 : (1 + SRSRAN_SIRNTI));
    if (!q->users) {
      perror("malloc");
      goto clean_exit;
    }

    if (srsran_sequence_init(&q->tmp_seq, 20)) {
      goto clean_exit;
    }
...
void srsran_pucch_free(srsran_pucch_t* q)
{
  if (q->users) {
    if (q->is_ue) {
      srsran_pucch_free_rnti(q, 0);
    } else {
      for (int rnti = 0; rnti <= SRSRAN_SIRNTI; rnti++) {
        srsran_pucch_free_rnti(q, rnti);
      }
    }
    free(q->users);
  }
...
void srsran_pucch_free_rnti(srsran_pucch_t* q, uint16_t rnti)
{
  uint32_t rnti_idx = q->is_ue ? 0 : rnti;

  if (q->users[rnti_idx]) {
    for (int i = 0; i < SRSRAN_NOF_SF_X_FRAME; i++) {
      srsran_sequence_free(&q->users[rnti_idx]->seq_f2[i]);
    }
...
int srsran_pucch_set_rnti(srsran_pucch_t* q, uint16_t rnti)
{

  uint32_t rnti_idx = q->is_ue ? 0 : rnti;
  if (!q->users[rnti_idx] || q->is_ue) {
    if (!q->users[rnti_idx]) {
      q->users[rnti_idx] = calloc(1, sizeof(srsran_pucch_user_t));
      if (!q->users[rnti_idx]) {
        perror("calloc");
        return -1;
      }
    ...
    for (uint32_t sf_idx = 0; sf_idx < SRSRAN_NOF_SF_X_FRAME; sf_idx++) {
      ...
      if (srsran_sequence_pucch(&q->users[rnti_idx]->seq_f2[sf_idx], rnti, 2 * sf_idx, q->cell.id)) {
...
static srsran_sequence_t* get_user_sequence(srsran_pucch_t* q, uint16_t rnti, uint32_t sf_idx)
{
  uint32_t rnti_idx = q->is_ue ? 0 : rnti;

  // The scrambling sequence is pregenerated for all RNTIs in the eNodeB but only for C-RNTI in the UE
  if (rnti >= SRSRAN_CRNTI_START && rnti < SRSRAN_CRNTI_END) {
    if (q->users[rnti_idx] && q->users[rnti_idx]->sequence_generated && q->users[rnti_idx]->cell_id == q->cell.id &&
      ...
      if (srsran_sequence_pucch(&q->tmp_seq, rnti, 2 * sf_idx, q->cell.id)) {
        ERROR("Error computing PUCCH Format 2 scrambling sequence\n");
        return NULL;
      }
      return &q->tmp_seq;
...
uci_mod_bits(srsran_pucch_t* q, srsran_ul_sf_cfg_t* sf, srsran_pucch_cfg_t* cfg, uint8_t bits[SRSRAN_PUCCH_MAX_BITS])
{
  ...
  switch (cfg->format) {
    ...
    case SRSRAN_PUCCH_FORMAT_2B:
      ...
      if (seq) {
        memcpy(q->bits_scram, bits, SRSRAN_PUCCH2_NOF_BITS * sizeof(uint8_t));
        srsran_scrambling_b_offset(seq, q->bits_scram, 0, SRSRAN_PUCCH2_NOF_BITS);
    ...
    case SRSRAN_PUCCH_FORMAT_3:
      seq = get_user_sequence(q, cfg->rnti, sf->tti % 10);
      if (seq) {
        memcpy(q->bits_scram, bits, SRSRAN_PUCCH3_NOF_BITS * sizeof(uint8_t));
        srsran_scrambling_b_offset(seq, q->bits_scram, 0, SRSRAN_PUCCH3_NOF_BITS);
      ...
      break;
    default:
      ERROR("PUCCH format %s not supported\n", srsran_pucch_format_text(cfg->format));
      return SRSRAN_ERROR;
  }
  return SRSRAN_SUCCESS;
}
...
void srsran_refsignal_r_uv_arg_1prb(float* arg, uint32_t u);
                                  ...
                                  bool                signal_only)
{
  ...
  for (uint32_t ns = SRSRAN_NOF_SLOTS_PER_SF * sf_idx; ns < SRSRAN_NOF_SLOTS_PER_SF * (sf_idx + 1); ns++) {
    ...
    srsran_refsignal_r_uv_arg_1prb(q->tmp_arg, u);
    ...
    for (uint32_t m = 0; m < N_sf; m++) {
      ...
      if (cfg->format >= SRSRAN_PUCCH_FORMAT_2) {
        ...
        for (uint32_t n = 0; n < SRSRAN_PUCCH_N_SEQ; n++) {
          z[(ns % 2) * N_sf * SRSRAN_PUCCH_N_SEQ + m * SRSRAN_PUCCH_N_SEQ + n] =
              q->d[(ns % 2) * N_sf + m] * cexpf(I * (q->tmp_arg[n] + alpha * n));
        }
      } else {
        uint32_t n_prime_ns = 0;
        uint32_t n_oc       = 0;
        alpha      = srsran_pucch_alpha_format1(q->n_cs_cell, cfg, q->cell.cp, true, ns, l, &n_oc, &n_prime_ns);
        float S_ns = 0;
        if (n_prime_ns % 2) {
          S_ns = M_PI / 2;
        }
        ...
        for (uint32_t n = 0; n < SRSRAN_PUCCH_N_SEQ; n++) {
          z[(ns % 2) * N_sf_0 * SRSRAN_PUCCH_N_SEQ + m * SRSRAN_PUCCH_N_SEQ + n] =
                                 ...
                                 cf_t                z[SRSRAN_PUCCH_MAX_SYMBOLS])
{
  ...
  srsran_sequence_t* seq = get_user_sequence(q, cfg->rnti, sf->tti % 10);
  if (seq) {
    srsran_demod_soft_demodulate_s(SRSRAN_MOD_QPSK, q->d, q->llr, SRSRAN_PUCCH3_NOF_BITS);

    srsran_scrambling_s_offset(seq, q->llr, 0, SRSRAN_PUCCH3_NOF_BITS);

    return (int)srsran_uci_decode_m_basis_bits(q->llr, SRSRAN_PUCCH3_NOF_BITS, bits, SRSRAN_UCI_MAX_ACK_SR_BITS);
  } else {
    ERROR("Error modulating PUCCH3 bits: rnti not set\n");
    return SRSRAN_ERROR;
  }
                       ...
                       uint8_t               pucch2_bits[SRSRAN_PUCCH_MAX_BITS])
{
  if (format < SRSRAN_PUCCH_FORMAT_2) {
    ...
    srsran_uci_encode_m_basis_bits(temp, k, pucch_bits, SRSRAN_PUCCH3_NOF_BITS);
  }
  return SRSRAN_SUCCESS;
}
                          ...
                          float*              correlation)
{
  ...
  switch (cfg->format) {
    ...
    case SRSRAN_PUCCH_FORMAT_2A:
    case SRSRAN_PUCCH_FORMAT_2B:
      seq = get_user_sequence(q, cfg->rnti, sf->tti % SRSRAN_NOF_SF_X_FRAME);
      if (seq) {
        ...
        srsran_scrambling_s_offset(seq, llr_pucch2, 0, SRSRAN_PUCCH2_NOF_BITS);
...
uint32_t srsran_pucch_n_prb(srsran_cell_t* cell, srsran_pucch_cfg_t* cfg, uint32_t ns)
{
...
void srsran_pucch_rx_info(srsran_pucch_cfg_t* cfg, srsran_pucch_res_t* pucch_res, char* str, uint32_t str_len)
{
  ...
  if (pucch_res) {
    if (isnormal(cfg->threshold_dmrs_detection)) {
      n = srsran_print_check(str, str_len, n, ", dmrs_corr=%.3f", pucch_res->dmrs_correlation);
    }

    n = srsran_print_check(str, str_len, n, ", corr=%.3f", pucch_res->correlation);

    srsran_uci_data_info(&cfg->uci_cfg, &pucch_res->uci_data, &str[n], str_len - n);
  }
}
