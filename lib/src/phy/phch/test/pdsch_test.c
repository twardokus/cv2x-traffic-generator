                          ...
                          int                 tb)
{
  ...
  if (!pdsch_ue->llr_is_8bit && !tb_cw_swap) {
    ...
    // Generate sequence
    srsran_sequence_pdsch(&pdsch_ue->tmp_seq,
                          rnti,
    ...
    // Scramble
    srsran_scrambling_s_offset(&pdsch_ue->tmp_seq, pdsch_ue->e[tb], 0, pdsch_cfg->grant.tb[tb].nof_bits);
...
int main(int argc, char** argv)
{
  ...
  srsran_pdsch_set_rnti(&pdsch_rx, rnti);
  ...
  if (input_file) {
    ...
    srsran_pdsch_set_rnti(&pdsch_tx, rnti);
  ...
  for (uint32_t i = 0; i < SRSRAN_MAX_CODEWORDS; i++) {
    ...
    INFO("         mcs.mod=%s\n", srsran_mod_string(pdsch_cfg.grant.tb[i].mod));
