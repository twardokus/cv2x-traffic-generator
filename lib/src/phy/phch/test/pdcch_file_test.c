...
int main(int argc, char** argv)
{
  ...
  int                   frame_cnt;
  int                   ret;
  srsran_dci_location_t locations[MAX_CANDIDATES];
  ...
  do {
    ...
    if (rnti == SRSRAN_SIRNTI) {
      ...
      nof_locations = srsran_pdcch_common_locations(&pdcch, locations, MAX_CANDIDATES, cfi);
    } else {
      INFO("Initializing user-specific search space for RNTI: 0x%x\n", rnti);
      nof_locations = srsran_pdcch_ue_locations(&pdcch, &dl_sf, locations, MAX_CANDIDATES, rnti);
    }
