...
static int pdsch_init(srsran_pdsch_t* q, uint32_t max_prb, bool is_ue, uint32_t nof_antennas)
{
  ...
  if (q != NULL) {
    ...
    for (int i = 0; i < SRSRAN_MAX_CODEWORDS; i++) {
      ...
      if (is_ue) {
        q->evm_buffer[i] = srsran_evm_buffer_alloc(6);
        if (!q->evm_buffer[i]) {
    ...
    q->users = calloc(sizeof(srsran_pdsch_user_t*), q->is_ue ? 1 : (1 + SRSRAN_SIRNTI));
    if (!q->users) {
      ERROR("malloc");
      goto clean;
    }

    if (srsran_sequence_init(&q->tmp_seq, q->max_re * srsran_mod_bits_x_symbol(SRSRAN_MOD_256QAM))) {
      goto clean;
    }
...
void srsran_pdsch_free(srsran_pdsch_t* q)
{
  ...
  if (q->users) {
    if (q->is_ue) {
      srsran_pdsch_free_rnti(q, 0);
    } else {
      for (int u = 0; u <= SRSRAN_SIRNTI; u++) {
        if (q->users[u]) {
          srsran_pdsch_free_rnti(q, u);
        }
      }
    }
    free(q->users);
...
int srsran_pdsch_set_cell(srsran_pdsch_t* q, srsran_cell_t cell)
{
  ...
  if (q != NULL && srsran_cell_isvalid(&cell)) {
    ...
    if (q->is_ue) {
      for (int i = 0; i < SRSRAN_MAX_CODEWORDS; i++) {
        srsran_evm_buffer_resize(q->evm_buffer[i], cell.nof_prb);
      }
    }
 ...
 */
int srsran_pdsch_set_rnti(srsran_pdsch_t* q, uint16_t rnti)
{
  uint32_t rnti_idx = q->is_ue ? 0 : rnti;

  if (!q->users[rnti_idx] || q->is_ue) {
    if (!q->users[rnti_idx]) {
      q->users[rnti_idx] = calloc(1, sizeof(srsran_pdsch_user_t));
      if (!q->users[rnti_idx]) {
        ERROR("calloc");
        return -1;
      }
    ...
    for (int i = 0; i < SRSRAN_NOF_SF_X_FRAME; i++) {
      for (int j = 0; j < SRSRAN_MAX_CODEWORDS; j++) {
        if (srsran_sequence_pdsch(&q->users[rnti_idx]->seq[j][i],
                                  rnti,
                                  j,
                                  2 * i,
                                  q->cell.id,
                                  q->max_re * srsran_mod_bits_x_symbol(SRSRAN_MOD_256QAM))) {
          ERROR("Error initializing PDSCH scrambling sequence\n");
          srsran_pdsch_free_rnti(q, rnti);
          return SRSRAN_ERROR;
        }
      }
    }
...
void srsran_pdsch_free_rnti(srsran_pdsch_t* q, uint16_t rnti)
{
  uint32_t rnti_idx = q->is_ue ? 0 : rnti;
  if (q->users[rnti_idx]) {
    for (int i = 0; i < SRSRAN_NOF_SF_X_FRAME; i++) {
      for (int j = 0; j < SRSRAN_MAX_CODEWORDS; j++) {
        srsran_sequence_free(&q->users[rnti_idx]->seq[j][i]);
      }
    }
...
static srsran_sequence_t*
get_user_sequence(srsran_pdsch_t* q, uint16_t rnti, uint32_t codeword_idx, uint32_t sf_idx, uint32_t len)
{
  uint32_t rnti_idx = q->is_ue ? 0 : rnti;
  ...
  if (q->users[rnti_idx] && q->users[rnti_idx]->sequence_generated && q->users[rnti_idx]->cell_id == q->cell.id &&
    ...
    srsran_sequence_pdsch(&q->tmp_seq, rnti, codeword_idx, 2 * sf_idx, q->cell.id, len);
    return &q->tmp_seq;
  }
}
                                        ...
                                        bool*               ack)
{
  ...
  if (softbuffer && data && ack && cfg->grant.tb[tb_idx].nof_bits && cfg->grant.nof_re) {
    ...
    /* Bit scrambling */
    if (q->llr_is_8bit) {
      srsran_scrambling_sb_offset(seq, q->e[codeword_idx], 0, cfg->grant.tb[tb_idx].nof_bits);
    } else {
      srsran_scrambling_s_offset(seq, q->e[codeword_idx], 0, cfg->grant.tb[tb_idx].nof_bits);
    }
                                        ...
                                        uint32_t                nof_layers)
{
  ...
  if (cfg->grant.tb[tb_idx].enabled) {
    ...
    /* Bit scrambling */
    srsran_scrambling_bytes(seq, (uint8_t*)q->e[codeword_idx], cfg->grant.tb[tb_idx].nof_bits);
...
srsran_pdsch_rx_info(srsran_pdsch_cfg_t* cfg, srsran_pdsch_res_t res[SRSRAN_MAX_CODEWORDS], char* str, uint32_t str_len)
{
  ...
  if (cfg->meas_time_en) {
    len = srsran_print_check(str, str_len, len, ", t=%d us\n", cfg->meas_time_value);
  }
