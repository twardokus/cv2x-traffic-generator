...
static int chest_sl_init(srsran_chest_sl_t* q, uint32_t nof_cyclic_shift_seq)
{
  ...
  for (int i = 0; i < SRSRAN_SL_MAX_DMRS_SYMB; i++) {
    q->r[i] = srsran_vec_f_malloc(SRSRAN_MAX_PRB * SRSRAN_NRE);
    if (!q->r[i]) {
      ERROR("Error allocating memory");
      return SRSRAN_ERROR;
    }

    q->r_uv[i] = srsran_vec_cf_malloc(SRSRAN_MAX_PRB * SRSRAN_NRE);
    if (!q->r_uv[i]) {
      ERROR("Error allocating memory");
      return SRSRAN_ERROR;
    }
...
static int chest_sl_psbch_gen(srsran_chest_sl_t* q)
{
  ...
  for (int j = 0; j < q->nof_dmrs_symbols; ++j) {
    q->q[j]    = srsran_refsignal_get_q(u[j], SRSRAN_SL_BASE_SEQUENCE_NUMBER, N_zc);
