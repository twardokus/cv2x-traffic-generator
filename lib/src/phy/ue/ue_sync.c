...
static int track_peak_ok(srsran_ue_sync_t* q, uint32_t track_idx)
{
   ...
   */
  DEBUG(
      "TRACK: cfo_current=%f, cfo_strack=%f\n", 15000 * q->cfo_current_value, 15000 * srsran_sync_get_cfo(&q->strack));
...
static int track_peak_no(srsran_ue_sync_t* q)
{
  ...
  if (q->frame_no_cnt >= TRACK_MAX_LOST) {
    ...
    INFO("Tracking peak not found. Peak %.3f, %d lost\n", srsran_sync_get_peak_value(&q->strack), (int)q->frame_no_cnt);
    return 1;
  }
}
                                      ...
                                      const uint32_t    max_num_samples)
{
  ...
  while (align_len > q->sf_len) {
    ...
    actual_rx_len          = SRSRAN_MIN(align_len, actual_rx_len);
