...
void srsran_nbiot_ue_dl_get_sib1_grant(srsran_nbiot_ue_dl_t* q, uint32_t sfn, srsran_ra_nbiot_dl_grant_t* grant)
{
  ...
  srsran_ra_nbiot_dl_dci_to_grant(&ra_dl_sib1,
                                  grant,
                                  srsran_nbiot_ue_dl_get_next_sib1_start(q, sfn),
                                  4,
                                  DUMMY_R_MAX,
                                  true,
                                  q->cell.mode);
...
typedef struct {
  ...
  srsran_dci_location_t loc[MAX_CANDIDATES];
...
int srsran_nbiot_ue_dl_find_dl_dci_type_siprarnti(srsran_nbiot_ue_dl_t* q, uint16_t rnti, srsran_dci_msg_t* dci_msg)
{
  ...
  search_space.nof_locations = srsran_npdcch_common_locations(search_space.loc, MAX_CANDIDATES_COM);
  DEBUG(
                                              ...
                                              srsran_dci_msg_t*     dci_msg)
{
  ...
  dci_blind_search_t search_space;
  search_space.nof_locations = srsran_npdcch_ue_locations(search_space.loc, MAX_CANDIDATES_UE);
