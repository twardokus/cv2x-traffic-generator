...
int srsran_ue_ul_init(srsran_ue_ul_t* q, cf_t* out_buffer, uint32_t max_prb)
{
  ...
  if (q != NULL) {
    ...
    if (srsran_refsignal_ul_init(&q->signals, max_prb)) {
      ERROR("Error initiating srsran_refsignal_ul\n");
      goto clean_exit;
    }
...
void srsran_ue_ul_free(srsran_ue_ul_t* q)
{
  if (q) {
    ...
    srsran_refsignal_ul_free(&q->signals);
 ...
 */
void srsran_ue_ul_set_rnti(srsran_ue_ul_t* q, uint16_t rnti)
{
  srsran_pusch_set_rnti(&q->pusch, rnti);
  srsran_pucch_set_rnti(&q->pucch, rnti);
  q->current_rnti = rnti;
}
                                    ...
                                    srsran_pusch_grant_t* grant)
{
  if (srsran_ra_ul_dci_to_grant(&q->cell, sf, &cfg->ul_cfg.hopping, dci, grant)) {
    char str[128];
    srsran_dci_ul_info(dci, str, sizeof(str));
