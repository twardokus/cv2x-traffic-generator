...
int srsran_ue_dl_set_cell(srsran_ue_dl_t* q, srsran_cell_t cell)
{
  ...
  if (q != NULL && srsran_cell_isvalid(&cell)) {
    ...
    if (q->pregen_rnti) {
      srsran_ue_dl_set_rnti(q, q->pregen_rnti);
    }
    ret = SRSRAN_SUCCESS;
  } else {
 ...
 * For the connection procedure, use srsran_pusch_encode_rnti() or srsran_pusch_decode_rnti() functions
 */
void srsran_ue_dl_set_rnti(srsran_ue_dl_t* q, uint16_t rnti)
{

  srsran_pdsch_set_rnti(&q->pdsch, rnti);
  ...
  for (int i = 0; i < MI_NOF_REGS; i++) {
    srsran_pdcch_set_regs(&q->pdcch, &q->regs[i]);
    for (int cfi = 0; cfi < 3; cfi++) {
      sf_cfg.cfi = cfi + 1;
      for (int sf_idx = 0; sf_idx < 10; sf_idx++) {
        sf_cfg.tti                                     = sf_idx;
        q->current_ss_ue[i][cfi][sf_idx].nof_locations = srsran_pdcch_ue_locations(
            &q->pdcch, &sf_cfg, q->current_ss_ue[i][cfi][sf_idx].loc, MAX_CANDIDATES_UE, rnti);
      }
      q->current_ss_common[i][cfi].nof_locations =
          srsran_pdcch_common_locations(&q->pdcch, q->current_ss_common[i][cfi].loc, MAX_CANDIDATES_COM, cfi + 1);
    }
  }
  q->pregen_rnti = rnti;
}
                            ...
                            srsran_dci_msg_t    dci_msg[SRSRAN_MAX_DCI_MSG])
{
  ...
  if (rnti) {
    int i = 0;
    while ((dci_cfg->cif_enabled || !nof_dci) && (i < search_space->nof_locations) && (nof_dci < SRSRAN_MAX_DCI_MSG)) {
      ...
      if ((dci_msg[nof_dci].rnti == rnti) && (dci_msg[nof_dci].nof_bits > 0)) {
        ...
        if (dci_msg[nof_dci].format == SRSRAN_DCI_FORMAT0 && search_space->format == SRSRAN_DCI_FORMAT1A) {
          /* If there is space for accumulate another UL DCI dci and it was not detected before, then store it */
          if (q->pending_ul_dci_count < SRSRAN_MAX_CARRIERS &&
  ...
  if (rnti) {
    ...
    if (q->pending_ul_dci_count) {
      ...
      if (q->pregen_rnti == rnti) {
        ...
        current_ss->nof_locations = srsran_pdcch_ue_locations(&q->pdcch, sf, current_ss->loc, MAX_CANDIDATES_UE, rnti);
      }

      current_ss->format = SRSRAN_DCI_FORMAT0;
                                      ...
                                      srsran_dci_msg_t    dci_msg[SRSRAN_MAX_DCI_MSG])
{
  ...
  srsran_dci_cfg_t dci_cfg = dl_cfg->cfg.dci;

  // Configure and run DCI blind search
  dci_blind_search_t search_space;
  search_space.nof_locations = srsran_pdcch_common_locations(&q->pdcch, search_space.loc, MAX_CANDIDATES_COM, sf->cfi);
...
static int find_dl_dci_type_crnti(srsran_ue_dl_t*     q,
                                  srsran_dl_sf_cfg_t* sf,
                                  srsran_ue_dl_cfg_t* cfg,
                                  uint16_t            rnti,
                                  srsran_dci_msg_t    dci_msg[SRSRAN_MAX_DCI_MSG])
{
  ...
  if (q->pregen_rnti == rnti) {
    ...
    // If locations are not pre-generated, generate them now
    current_ss->nof_locations = srsran_pdcch_common_locations(&q->pdcch, current_ss->loc, MAX_CANDIDATES_COM, cfi);
  }

  // Search for RNTI only if there is room for the common search space
  if (current_ss->nof_locations > 0) {
    current_ss->format = SRSRAN_DCI_FORMAT1A;
  ...
  if (cfg->cfg.tm > SRSRAN_TM8) {
    ERROR("Searching DL CRNTI: Invalid TM=%d\n", cfg->cfg.tm + 1);
    return SRSRAN_ERROR;
  }

  for (int f = 0; f < 2; f++) {
    srsran_dci_format_t format = ue_dci_formats[cfg->cfg.tm][f];

    INFO("Searching DL C-RNTI %s in %d ue locations\n", srsran_dci_format_string(format), current_ss->nof_locations);
