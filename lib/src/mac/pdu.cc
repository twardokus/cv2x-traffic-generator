...
{
  return write_packet(srsran::log_ref{"MAC "});
}

/* Writes the MAC PDU in the packet, including the MAC headers and CE payload. Section 6.1.2 */
uint8_t* sch_pdu::write_packet(srsran::log_ref log_h)
{
...
int sch_pdu::get_sdu_space()
{
  ...
  ret = SRSRAN_MIN(ret >= 0 ? ret : 0, buffer_tx->get_tailroom());
  return ret;
}
...
rar_pdu::rar_pdu(uint32_t max_rars_, srsran::log_ref log_) : pdu(max_rars_, log_)
{
  backoff_indicator     = 0;
...
void rar_subh::fprint(FILE* stream)
{
  ...
  srsran_vec_fprint_hex(stream, grant, 20);
}
